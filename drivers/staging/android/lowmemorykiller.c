...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (nr_to_scan > 0)
		lowmem_print(3, "lowmem_shrink %lu, %x, ofree %d %d, ma %hd, rfree %d\n",
	...
	for_each_process(tsk) {
		...
		lowmem_print(2, "select %d (%s), oom_adj %d score_adj %hd, size %d, to kill\n",
			     p->pid, p->comm, selected_oom_adj, oom_score_adj, tasksize);
	}
	if (selected) {
		lowmem_print(1, "[%s] send sigkill to %d (%s), oom_adj %hd, score_adj %d,"
